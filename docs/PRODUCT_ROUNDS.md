...
| 2026-08-27 | **Parquet sharding convention** | All parquet now `<dir>/<table>/part-NNNNNN.parquet`, each part ≤40 MB (under GitHub limits, no LFS). `scripts/shard_parquet.py` + CI `--verify-only`; writers shard automatically. See `docs/DATA_PARQUET.md`. |
